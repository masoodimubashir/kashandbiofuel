...
<div class="product-box-slider-2 no-arrow">
    @for ($section = 0; $section < 5; $section++)
        <div>
            @foreach ($allProducts->skip($section * 2)->take(2) as $product)
                <div class="product-box product-box-bg wow fadeInUp" data-wow-delay="0.1s">
                    ...
                    <div class="product-detail">
                        ...
                        <h5 class="sold text-content">
                            <span
                                class="theme-color price">{{ Number::currency($product->selling_price, 'INR') }}</span>
                        ...
                        <div class="product-rating mt-2">
                            <ul class="rating">
                                @foreach (range(1, 5) as $star)
                                    <li>
                                        <i data-feather="star"
                                            class="{{ $star <= $product->rating ? 'fill' : '' }}"></i>
                                    </li>
                                @endforeach
                            </ul>
